"""Deterministic reconstruction of persisted CYDRA causal reasoning chains."""
...
class CausalChainTrace:
    ...
    def evidence_ids(self) -> tuple[str, ...]:
        """Canonical references carried by this causal trace."""
...
def reconstruct_causal_chain(model: SystemModel, chain_id: str) -> CausalChainTrace:
    """Reconstruct one persisted chain and reject ambiguous or semantically broken links."""
    ...
    audit_session_id = chain.attributes.get("audit_session_id")
    if audit_session_id is not None:
        if not isinstance(audit_session_id, str) or not audit_session_id.strip():
            raise ValueError(f"causal chain audit-session identity is malformed: {chain_id}")
        session = model.nodes.get(audit_session_id)
        if session is None or session.kind != "audit_session":
            raise ValueError(f"causal chain audit-session reference is not canonical: {chain_id}")

    def targets(source: str, relation: str) -> list[str]:
        return [
            e.target for e in model.edges
            if e.source == source
            and e.relation == relation
            and e.attributes.get("causal_chain_id") == chain_id
        ]
    ...
    motivating = [
        e.source for e in model.edges
        if e.target == chain_id
        and e.relation == "motivates"
        and e.attributes.get("causal_chain_id") == chain_id
    ]
    if len(motivating) != 1:
    ...
    expected = {
        (hypothesis_id, "motivates", chain_id),
        (chain_id, "plans", observation_id),
        (observation_id, "produced_evidence", evidence_id),
        (evidence_id, "informs", verification_id),
        (verification_id, "updates", belief_update_id),
    }
    for source, relation, target_id in expected:
        matching = [
            e for e in model.edges
            if e.source == source and e.relation == relation and e.target == target_id
            and e.attributes.get("causal_chain_id") == chain_id
        ]
        if len(matching) != 1:
            raise ValueError(f"causal-chain link is missing or duplicated: {source} -[{relation}]-> {target_id}")

    return CausalChainTrace(
        chain_id,
        hypothesis_id,
        observation_id,
        evidence_id,
