...
This layer records belief changes without treating verification as proof. Every update
retains the evidence IDs and prior belief, so downstream reasoning can audit how a
hypothesis moved. Planner predictions are carried only as explicit planning metadata;
they are never inferred from persistent belief or verification state.
"""
...
class Hypothesis:
    ...
    @property
    def name(self) -> str:
        """Compatibility view used by older persistence callers."""
        prefix = "hypothesis:"
...
def update_hypothesis(
    ...
    """Update belief conservatively from an explicit candidate verification.

    The update is deliberately bounded: support moves belief toward 1, contradiction
    moves it toward 0, and unresolved evidence leaves belief unchanged. It never emits
    a certainty claim or converts a hypothesis into a confirmed invariant.
    """
    ...
    if verification.state == VerificationState.SUPPORTED and relevant:
        strength = max(e.confidence for e in relevant)
